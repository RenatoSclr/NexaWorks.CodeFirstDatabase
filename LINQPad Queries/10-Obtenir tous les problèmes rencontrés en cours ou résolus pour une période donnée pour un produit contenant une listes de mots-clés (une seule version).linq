...
void Main()
{
	...
	int? produitId = 3; 
	double? versionNumero = 2.0; 
	
	DateTime dateDebut = new DateTime(2023, 5, 1);
    DateTime dateFin = new DateTime(2023, 11, 20);
	
	string[] motsCles = ["entraînement"]; 
	
	IQueryable<Tickets> tickets = Tickets.AsQueryable();
	
    
    var query = TicketExtensions.GetQuery(tickets, estResolu, produitId, versionNumero, motsCles);
